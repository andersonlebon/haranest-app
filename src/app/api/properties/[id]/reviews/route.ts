...
export async function GET(
  ...
  try {
    ...
    return createErrorResponse(error, 500, 'Failed to fetch reviews');
  }
}
  ...
  try {
    ...
    if (!name || !comment || rating < 1 || rating > 5) {
      return NextResponse.json(
        { error: 'Name, comment and rating(1-5) are required' },
        { status: 400 }
      );
    }
    ...
    return createErrorResponse(error, 500, 'Failed to create review');
  }
}
